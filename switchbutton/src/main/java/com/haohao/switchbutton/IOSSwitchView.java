...
/**
 * fork on https://github.com/iielse/SwitchButton
 */
public class IOSSwitchView extends View {
    ...
    public IOSSwitchView(Context context, AttributeSet attrs) {
        ...
        colorPrimary = a.getColor(R.styleable.IOSSwitchView_primaryColor, DEFAULT_COLOR_PRIMARY);
        colorPrimaryDark =
                a.getColor(R.styleable.IOSSwitchView_primaryColorDark, DEFAULT_COLOR_PRIMARY_DARK);
        colorOff = a.getColor(R.styleable.IOSSwitchView_offColor, 0xFFE3E3E3);
        colorOffDark = a.getColor(R.styleable.IOSSwitchView_offColorDark, 0xFFBFBFBF);
        colorShadow = a.getColor(R.styleable.IOSSwitchView_shadowColor, 0xFF333333);
        ratioAspect = a.getFloat(R.styleable.IOSSwitchView_ratioAspect, 0.68f);
        hasShadow = a.getBoolean(R.styleable.IOSSwitchView_hasShadow, true);
        isOpened = a.getBoolean(R.styleable.IOSSwitchView_isOpened, false);
        isEnable = a.getBoolean(R.styleable.IOSSwitchView_isEnable, true);

        state = isOpened ? STATE_SWITCH_ON : STATE_SWITCH_OFF;
        lastState = state;
        a.recycle();

        if (colorPrimary == DEFAULT_COLOR_PRIMARY
                && colorPrimaryDark == DEFAULT_COLOR_PRIMARY_DARK) {
            try {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    TypedValue primaryColorTypedValue = new TypedValue();
                    context.getTheme()
                            .resolveAttribute(android.R.attr.colorPrimary, primaryColorTypedValue,
                                    true);
                    ...
                    context.getTheme()
                            .resolveAttribute(android.R.attr.colorPrimaryDark,
                                    primaryColorDarkTypedValue, true);
    ...
    public void setColor(int newColorPrimary, int newColorPrimaryDark, int newColorOff,
            int newColorOffDark) {
        setColor(newColorPrimary, newColorPrimaryDark, newColorOff, newColorOffDark, colorShadow);
    }

    public void setColor(int newColorPrimary, int newColorPrimaryDark, int newColorOff,
            int newColorOffDark, int newColorShadow) {
    ...
    public void toggleSwitch(boolean isOpened) {
        ...
        if ((wishState == STATE_SWITCH_ON && (state == STATE_SWITCH_OFF
                || state == STATE_SWITCH_OFF2)) || (wishState == STATE_SWITCH_OFF && (state
                == STATE_SWITCH_ON || state == STATE_SWITCH_ON2))) {
            sAnim = 1;
        }
        bAnim = 1;
        refreshState(wishState);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        isCanVisibleDrawing = w > getPaddingLeft() + getPaddingRight()
        ...
        if (isCanVisibleDrawing) {
            ...
            if (actuallyDrawingAreaWidth * ratioAspect < actuallyDrawingAreaHeight) {
                ...
                int heightExtraSize =
                        (int) (actuallyDrawingAreaHeight - actuallyDrawingAreaWidth * ratioAspect);
                ...
                int widthExtraSize =
                        (int) (actuallyDrawingAreaWidth - actuallyDrawingAreaHeight / ratioAspect);
            ...
            shadowReservedHeight =
                    (int) ((actuallyDrawingAreaBottom - actuallyDrawingAreaTop) * 0.07f);
            ...
            shadowGradient = new RadialGradient(bCenterX, bCenterY, bRadius,
                    Color.argb(200, red, green, blue), Color.argb(25, red, green, blue),
    ...
    protected void onDraw(Canvas canvas) {
        ...
        paint.setColor(isEnable ? 0xFFFFFFFF : 0xffe4e4e4);
        canvas.drawPath(sPath, paint);
        canvas.restore();
        ...
        paint.setStyle(Paint.Style.FILL);
        paint.setColor(isEnable ? 0xffffffff : 0xffB3B3B3);
        canvas.drawPath(bPath, paint);
    ...
    public interface OnStateChangedListener {
        void toggleToOn(IOSSwitchView view);

        void toggleToOff(IOSSwitchView view);
    }
